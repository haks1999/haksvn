/*
 * Test harness for DiffMatchPatchUtils.java
 *
...
import com.haks.haksvn.source.util.DiffMatchPatchUtils.LinesToCharsResult;
import com.haks.haksvn.source.util.DiffMatchPatchUtils.Patch;


public class DiffMatchPatchUtilsTest extends TestCase {

  private DiffMatchPatchUtils dmp;
  private DiffMatchPatchUtils.Operation DELETE = DiffMatchPatchUtils.Operation.DELETE;
  private DiffMatchPatchUtils.Operation EQUAL = DiffMatchPatchUtils.Operation.EQUAL;
  private DiffMatchPatchUtils.Operation INSERT = DiffMatchPatchUtils.Operation.INSERT;

  protected void setUp() {
    // Create an instance of the DiffMatchPatchUtils object.
    dmp = new DiffMatchPatchUtils();
  }
  ...
  private static String[] diff_rebuildtexts(LinkedList<Diff> diffs) {
    ...
    for (Diff myDiff : diffs) {
      if (myDiff.operation != DiffMatchPatchUtils.Operation.INSERT) {
        text[0] += myDiff.text;
      }
      if (myDiff.operation != DiffMatchPatchUtils.Operation.DELETE) {
        text[1] += myDiff.text;
      }
    }
    return text;
