 ...
 * 
 */
public class RetrieveFileLogForRevisionGraphTest {
    /*
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String trunkPath = "/trunk/src/main/java/com/haks/haksvn/repository/service/SVNRepositoryService.java";
            String tagsPath = "/tags/0.0.0-20130310-changedetail/src/main/java/com/haks/haksvn/repository/service/SVNRepositoryService.java";
            
            SVNDirEntry trunkEntry = repository.info(trunkPath, -1);
            SVNDirEntry taggedEntry = repository.info(tagsPath, -1);
            
            Collection<SVNLogEntry> entries = repository.log(new String[] { tagsPath },
            		null, 0,
            		-1,
            		true, true);
            //Map<String,List<SVNLogEntry>> changesByFileMap = new HashMap<String,List<SVNLogEntry>>();
            
			for (SVNLogEntry entry : entries) {
				Map<String,SVNLogEntryPath> map = entry.getChangedPaths();
				for( Map.Entry<String, SVNLogEntryPath> elem : map.entrySet() ){
					String copiedPath = elem.getValue().getCopyPath();
					if( copiedPath != null ){
						System.out.println(elem.getValue().getCopyRevision() + " : "+elem.getValue().getCopyPath());
					}
            ...
            Collection<SVNLogEntry> entries = repository.log(new String[] { trunkPath },
            		null, taggedEntry.getRevision(),
            		trunkEntry.getRevision(),
            		true, true);
            Map<String,List<SVNLogEntry>> changesByFileMap = new HashMap<String,List<SVNLogEntry>>();
			...
			for (SVNLogEntry entry : entries) {
				Set<String> pathSet = entry.getChangedPaths().keySet();
				
				Map<String,SVNLogEntryPath> map = entry.getChangedPaths();
				
				
				for (String path : pathSet) {
					if( path.indexOf(trunkPath) < 0 ) continue;
					List<SVNLogEntry> entryList = changesByFileMap.get(path);
					if (entryList == null) {
						entryList = new ArrayList<SVNLogEntry>();
						changesByFileMap.put(path, entryList);
					}
					entryList.add(entry);
				}
			}
            
			Set<String> pathSet = changesByFileMap.keySet();
			for (String path : pathSet) {
				System.out.println("Path: " + path);
				List<SVNLogEntry> entryList = changesByFileMap.get(path);

				for (SVNLogEntry logEntry : entryList) {
			...
			*/
            ...
            String[] filePathList = {"/tags/0.0.0-20130122/pom.xml"};
            long startRevision = 0;
            long endRevision = -1;//HEAD (the latest) revision
            
            final List<SVNLogEntry> tempList = new ArrayList<SVNLogEntry>();
            
            repository.log(filePathList, 0, -1, true, true, 100, new ISVNLogEntryHandler() { 
            ...
            for( SVNLogEntry svnLogEntry : tempList ){
            	...
            	for( Map.Entry<String, SVNLogEntryPath> elem : map.entrySet() ){
            		if( filePath.indexOf(elem.getKey()) > -1){
