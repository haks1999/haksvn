...
public class TraceSourceService {
	...
	public TraceSourceBasket retrieveTraceSourceListByPath(String repositoryKey, String path, int limit){
		...
		if( traceSourceForTransferList == null || traceSourceForTransferList.size() < 1 ) return new TraceSourceBasket();
		...
		Set<String> trunkPathSet = new HashSet<String>(0);
		Set<String> branchPathSet = new HashSet<String>(0);
		for( TraceSourceForTransfer traceSource : traceSourceForTransferList ){
			trunkPathSet.add(traceSource.getTrunkPath());
			branchPathSet.add(traceSource.getBranchesPath());
		...
		if( traceSourceForTransferList != null && traceSourceForTransferList.size() > 0 ){
			...
			for( Tagging tagging : taggingList ){
				...
				if( isTagged && !tagElemMap.containsKey(tagElemId)){	// restore와 create는 어차피 동일 tag 바라봄. create 타입만 해도 됨
					tagElemMap.put(tagElemId, TraceSourceElement.Builder.getBuilder().id(tagElemId).isTag(true).revision((isTagged?tagging.getDestRevision():tagging.getSrcRevision())).tagName(tagging.getTagName()).build());
				}
		...
		basket.setTrunkPathList(new ArrayList<String>(trunkPathSet));
		basket.setBranchPathList(new ArrayList<String>(branchPathSet));
